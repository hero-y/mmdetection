...
class FeatureAdaption(nn.Module):
    ...
    def forward(self,x,pre):
        offset = self.conv_offset(pre.detach())
...
class CascadeFCOSHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_centerness=dict(
                     ...
                     use_sigmoid=True,
                     loss_weight=1.0),
                 loss_bbox_dconv=dict(type='IoULoss', loss_weight=1.0),
                 conv_cfg=None,
        ...
        self.loss_bbox_dconv = build_loss(loss_bbox_dconv)
        ...
        for i in range(self.stacked_convs):
            ...
            self.reg_convs.append(
                ConvModule(
                    ...
                    norm_cfg=self.norm_cfg,
                    bias=self.norm_cfg is None))
        self.fcos_reg_dconv = nn.Conv2d(self.feat_channels, 4, 3, padding=1)#
    ...
    def init_weights(self):
        ...
        normal_init(self.fcos_reg_dconv, std=0.01) #
    ...
    def forward_single(self, x, scale):
        ...
        bbox_pred_dconv = scale(self.fcos_reg_dconv(reg_feat)).float().exp()
        cls_feat = self.feature_adaption_cls(cls_feat, bbox_pred_dconv)
        reg_feat = self.feature_adaption_reg(reg_feat, bbox_pred_dconv)
        cls_score = self.fcos_cls(cls_feat)
        # centerness = self.fcos_centerness(cls_feat)
        bbox_pred = scale(self.fcos_reg(reg_feat)).float().exp()

        return cls_score, bbox_pred, centerness, bbox_pred_dconv

    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'centernesses', 'bbox_pred_dconves'))
    def loss(self,
             cls_scores,
             bbox_preds,
             centernesses,
        ...
        flatten_bbox_preds_dconves = [
            bbox_pred_dconv.permute(0, 2, 3, 1).reshape(-1, 4)
            for bbox_pred_dconv in bbox_pred_dconves
        ]#
        #对pred和target都把5个level的值cat到一起
        flatten_cls_scores = torch.cat(flatten_cls_scores)
        flatten_bbox_preds = torch.cat(flatten_bbox_preds)
        flatten_centerness = torch.cat(flatten_centerness)
        flatten_bbox_preds_dconves = torch.cat(flatten_bbox_preds_dconves)
        ...
        pos_bbox_preds_dconves = flatten_bbox_preds_dconves[pos_inds]
        ...
        if num_pos > 0:
            ...
            pos_decoded_target_preds = distance2bbox(pos_points,
                                                     pos_bbox_targets)
            pos_decoded_bbox_preds_dconv = distance2bbox(pos_points, pos_bbox_preds_dconves)                                        
            ...
            loss_bbox_dconv = self.loss_bbox_dconv(
                pos_decoded_bbox_preds_dconv,
                pos_decoded_target_preds,
                weight=pos_centerness_targets,
                avg_factor=pos_centerness_targets.sum())                                       
        else:
        ...
        return dict(
            loss_cls=loss_cls,
            loss_bbox=0.5*loss_bbox,
            loss_centerness=loss_centerness,
            loss_bbox_dconv=loss_bbox_dconv)
    ...
    def fcos_target_single(self, gt_bboxes, gt_labels, points, regress_ranges):
        ...
        bbox_targets_center = bbox_targets
    ...
    #     left_right = pos_bbox_targets[:, [0, 2]]
    #     top_bottom = pos_bbox_targets[:, [1, 3]]
    #     area = (left_right[:,0] + left_right[:,1]) * (top_bottom[:,0] + top_bottom[:,1])
    #     min_left_right ,_ = left_right.min(dim=-1)
    #     min_top_bottom ,_ = top_bottom.min(dim=-1)
    #     min_area = min_left_right * min_top_bottom
    #     centerness_targets = (min_area/area)/0.25
    #     return torch.sqrt(centerness_targets)
