...
class Collect(object):
    ...
    def __init__(self,
        ...
        #ori_shape是最开始的图像的shape
        #img_shape是resize后的图像的shape
        #pad_shape是resize后并pad后的图像,pad是为了满足w,h都是size_divisor(32)的倍数
    def __call__(self, results):
        data = {}
        img_meta = {}
        for key in self.meta_keys:
            img_meta[key] = results[key]
        data['img_meta'] = DC(img_meta, cpu_only=True)#DC把这些值变成了tensor，并且放到了cuda上
        for key in self.keys:
