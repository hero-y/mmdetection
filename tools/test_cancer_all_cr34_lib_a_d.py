...
def main():
    ...
    if args.img_file is not None:
        ...
        for test_data_path in test_data_paths: #对每一副图像处理
            ...
            print("name",name)
            save_path = ("/home/hero-y/data/cancer/crops/tianchi_cr34_lib_a_d_overlap/"+ name).replace('.kfb','.json') 
            # print(save_path)
            save_data = []
            proposals_data = []
...
#             name = osp.basename(test_data_path)
#             print("name",name)
#             save_path = ("/home/hero-y/data/cancer/crops/tianchi_lf34/"+ name).replace('.kfb','.json') 
#             # print(save_path)
#             save_data = []
#             read.ReadInfo(test_data_path,_scale,False)
#             w = read.getWidth()
...
#             # print("w",w,"h",h)
#             # print(int(h/base_h),int(w/base_w))
#             for j in range(int(h/base_h)):#对每个ROI遍历
#                 for i in range(int(w/base_w)):
#                     ROI = read.ReadRoi(i*base_w, j*base_h, base_w, base_h, _scale)           
#                     result = inference_detector(model, ROI) #也是要经过pipeline的
#                     bbox_result = result
#                     bboxes = np.vstack(bbox_result) #pos是左上,右下的四个点(x,y,x,y)neg则是[],
#                     if len(bboxes):#有值
#                         for bbox in bboxes:
#                             roi_save_data = {}
#                             roi_save_data["x"] = (bbox[0] + i*base_w).item() #必须要加.item()
#                             roi_save_data["y"] = (bbox[1] + j*base_h).item()
#                             roi_save_data["w"] = (bbox[2]- bbox[0]).item()
#                             roi_save_data["h"] = (bbox[3]- bbox[1]).item()
#                             roi_save_data["p"] = bbox[4].item()
#                             save_data.append(roi_save_data)
#                         # print(save_data)
#             with open(save_path, 'w') as f:
#                 json.dump(save_data, f)
