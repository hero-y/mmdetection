...
def main():
    ...
    if args.img_file is not None:
        ...
        for test_data_path in test_data_paths: #对每一副图像处理
            ...
            # save_path = ("/home/admin/jupyter/results/test_cancer_cr34/"+ name).replace('.kfb','.json') 
            save_path = ("/home/hero-y/results/test_cancer_cr34/"+ name).replace('.kfb','.json') 
            ...
            for roi_label in roi_labels:
                ...
                #         ###添加对每一副大图生成Candida类
                #         if w > 300 and h > 300:
                #             Candida_data_bbox = np.empty((5,))
                #             Candida_data_bbox[0] = i*stride_w
                #             Candida_data_bbox[1] = j*stride_h
                #             Candida_data_bbox[2] = i*stride_w + base_w
                #             Candida_data_bbox[3] = j*stride_h + base_h
                #             Candida_data_bbox[4] = 0.2
                #             Candida_data.append(Candida_data_bbox.reshape(1,-1))
                #             Candida_label = np.ones((1,), dtype=np.int32)*4
                #             Candida_data_label.append(Candida_label)

                # ###添加对每一副大图生成Candida类
                # proposals_data.extend(Candida_data)
                # labels_data.extend(Candida_data_label)
