...
def crop_samples(area, roi, name, data_root):
    ...
    for i in range(len(roi)): #对每个图像的每个roi中的每个gt做循环
        ...
        x2 = min(roi[i]['x1']+1, W-crop_size+1)  #roi[i][0] 他不希望裁剪的区域超过roi
        ...
        for cnt, crop in enumerate(pos_crops): #因为每个bbox都生成了4幅图，这是对每幅图做的遍历
            ...
            #TODO 针对超大图的情况，gt的坐标在裁剪区域的外面，这样求相对坐标的时候就会出现负数
...
if __name__ == '__main__':
    ...
    data_root = '/home/hero-y/train_data_split2/' 
    ...
    for pos_name in os.listdir(train['img_prefix']):
        ...
        for label in labels:
            ...
            elif label['class'] in ['ASC-H','ASC-US','HSIL','LSIL','Candida','Trichomonas']:
